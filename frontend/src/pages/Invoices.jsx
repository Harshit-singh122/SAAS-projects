...
export default function InvoicesPage() {
  ...
  function openInvoice(inv) {
    ...
    navigate(`/app/invoice/${inv.id}/preview`, { state: { invoice: found } });
  }
  ...
  async function handleGenerateFromAI(rawText) {
    ...
    try {
      ...
      if (API_BASE) {
        const token = await obtainToken();
        const aiRes = await fetch(`${API_BASE}/api/ai-invoice/generate`, {
          method: "POST",
          headers: {
        ...
        console.log("FULL AI RESPONSE:", aiInvoice);
console.log("AI CLIENT OBJECT:", aiInvoice.client);
        const mappedInvoice = {
          invoiceNumber: `INV-${Math.floor(100000 + Math.random() * 900000)}`,
          issueDate: now,
          dueDate: aiInvoice?.dueDate || "",
          currency: "INR",
          status: "Draft",
          taxPercent: 18,
          notes: "",
          ...
          client: {
            name: aiInvoice?.client?.name || "",
            email: aiInvoice?.client?.email || "",

            phone:
              aiInvoice?.client?.phone ||
              aiInvoice?.client?.mobile ||
              aiInvoice?.client?.phoneNumber ||
              aiInvoice?.client?.contact ||
              aiInvoice?.client?.contactNumber ||
              aiInvoice?.phone ||
              "",

            address:
              aiInvoice?.client?.address ||
              aiInvoice?.client?.location ||
              aiInvoice?.client?.billingAddress ||
              aiInvoice?.clientAddress ||
              aiInvoice?.address ||
              "",
          },
          ...
          items: (aiInvoice?.items || []).map(item => ({
            ...
            qty: Number(item?.quantity || 1),        // ✅ matches backend
            unitPrice: Number(item?.price || 0),     // ✅ matches backend
          }))
        ...
        if (tokenForCreate) {
          const createRes = await fetch(`${API_BASE}/api/invoice`, {
            ...
            body: JSON.stringify(mappedInvoice),
          ...
          await fetchInvoices();
          setAiOpen(false);
          navigate(`/app/invoice/${saved.id}/edit`, {
            state: { invoice: saved },
          });
          return;
        } else {
      ...
      setAiOpen(false);
      navigate(`/app/invoice/${newId}/edit`, {
  ...
  return (
    <div className={invoicesStyles.pageContainer}>
      ...
      <AiInvoiceModal
        ...
        onGenerated={handleGenerateFromAI} /* <--- FIXED THIS PROP NAME */
        initialText=""
      />
    </div>
  );
